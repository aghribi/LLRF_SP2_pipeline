#!/usr/bin/env python3
"""
LLRF Anomaly Detection - Data Preparation for Cluster (V6 - LEAKAGE FIX: drop interlock_type)
================================================================================
...
class LLRFDataPreparation:
    ...
    def save_results(self, batch_dir_or_results):
        ...
        # V6: distinct filename -- do NOT overwrite V2/V3/V4/V5's outputs, all
        # real checkpoints. Callers switch to this explicitly once validated.
        features_file = self.output_dir / 'features_engineered_v6.pkl'
        with open(features_file, 'wb') as f:
            pickle.dump({
                ...
                'alignment_note': 'V4: per-file trigger alignment from Read_Signals time axis (zero_idx/dt_us), not hardcoded trigger_idx=3000. V5: additionally refines onset via AMPT (find_true_t0_with_ampt) for "Rég signal RF hors tolérance" events only. V6: additionally drops the interlock_type feature (ALM-derived, was a near-tautological label proxy -- see module docstring). See module docstring.',
        ...
        # Save summary (V6: distinct filename, don't overwrite V2/V3/V4/V5's summaries)
        summary_file = self.output_dir / 'processing_summary_v6.txt'
        with open(summary_file, 'w') as f:
            f.write(f"LLRF Data Preparation Summary (V6 -- V5 + interlock_type leakage fix)\n")
