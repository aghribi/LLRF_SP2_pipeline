#!/usr/bin/env python3
"""
LLRF Anomaly Detection - Data Preparation for Cluster (V4 - CORRECT TRIGGER ALIGNMENT)
================================================================================
...
def process_single_file_with_timeout(file_path, file_timeout=30, store_signals=False):
    ...
    try:
        ...
        # Preprocess signals
        signals_processed = preprocess_signals(signals, metadata, zero_idx, dt_us)

        # Engineer features
        features = engineer_features(signals_processed, metadata, zero_idx, dt_us)
...
class LLRFDataPreparation:
    ...
    def save_results(self, batch_dir_or_results):
        ...
        # V4: distinct filename -- do NOT overwrite V2's features_engineered.pkl
        # or V3's features_engineered_v3.pkl, both real checkpoints. Callers
        # switch to this explicitly once the new dataset is validated.
        features_file = self.output_dir / 'features_engineered_v4.pkl'
        with open(features_file, 'wb') as f:
            pickle.dump({
                ...
                'alignment_note': 'V4: per-file trigger alignment from Read_Signals time axis (zero_idx/dt_us), not hardcoded trigger_idx=3000. See module docstring.',
        ...
        # Save summary (V4: distinct filename, don't overwrite V2/V3's summaries)
        summary_file = self.output_dir / 'processing_summary_v4.txt'
        with open(summary_file, 'w') as f:
            f.write(f"LLRF Data Preparation Summary (V4 -- ground-truth labels + corrected trigger alignment)\n")
            f.write(f"=" * 80 + "\n")
