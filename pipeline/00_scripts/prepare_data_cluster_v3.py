#!/usr/bin/env python3
"""
LLRF Anomaly Detection - Data Preparation for Cluster (V3 - GROUND-TRUTH LABELS)
================================================================================
...
def engineer_features(signals_processed, metadata):
    ...
    - Physics features (44 total - CORRECTED PER THESIS TABLE 2.2):
      ...
      * Validity flags: 5 features (RF on, loop closed, beam present, interlock type, valid decay)
    ...
    if 'Ucav' in signals:
        (features['effective_decay_tau'],
         ...
         features['decay_non_exponentiality']) = calculate_effective_decay(signals['Ucav'], fs=fs)
    else:
    ...
    if 'A Ucr' in signals and 'A Uamp' in signals and 'PhaseCav' in signals:
        ...
        if len(rf_mismatch) > trigger_idx:
            ...
            # Reflected power saturation detection
            ucr = signals['A Ucr']
    ...
    if 'Uci' in signals and 'A Ucr' in signals:
        ...
        # Jump at trigger
        if len(forward_power) > trigger_idx + 10:
            pre_power = np.mean(forward_power[trigger_idx-10:trigger_idx])
            post_power = np.mean(forward_power[trigger_idx:trigger_idx+10])
    ...
    # RF drive state (from forward power)
    if 'Uci' in signals:
        forward_mean = np.mean(np.abs(signals['Uci']))
    ...
    # Beam presence (from metadata)
    beam_status = metadata.get('BEAM', 'UNKNOWN')
    if isinstance(beam_status, str):
        features['beam_present'] = int(beam_status.upper() in ['OUI', 'YES', 'ON'])
    else:
        features['beam_present'] = int(beam_status == 1)

    # Interlock type (from ALM bits)
    alm_value = metadata.get('ALM', 0)
    try:
        if isinstance(alm_value, str):
            alm = int(alm_value, 16) if alm_value else 0
        else:
            alm = int(alm_value) if alm_value else 0
    except:
        alm = 0

    # Get primary interlock (lowest bit set)
    features['interlock_type'] = -1  # Default: no interlock (manual acquisition)
    for bit in range(7):
        if (alm >> bit) & 1:
            features['interlock_type'] = bit
            break

    # Valid decay window (check if post-trigger has monotonic decay)
    if 'Ucav' in signals:
        trigger_idx = 3000
        if len(signals['Ucav']) > trigger_idx + 100:
            decay_window = np.abs(signals['Ucav'][trigger_idx:trigger_idx+100])
            # Check if mostly decreasing
            decreases = np.sum(np.diff(decay_window) < 0)
            features['valid_decay_window'] = int(decreases > 60)  # >60% samples decrease
        else:
...
def calculate_effective_decay(ucav_signal, fs=88000, decay_window_samples=500, nominal_tau=0.005):
    """
    Calculate effective system decay metrics (NOT Q_L)
...
class LLRFDataPreparation:
    ...
    def save_results(self, batch_dir_or_results):
        ...
        # Free fault_labels immediately (now stored in y_binary/y_multilabel/y_defaut/y_physics_subtype)
        del fault_labels
        gc.collect()
        ...
        # V3: distinct filename -- do NOT overwrite V2's features_engineered.pkl,
        # which the current report/notebooks still cite. Callers switch to this
        # explicitly once the new dataset is validated (see plan Phase 3).
        features_file = self.output_dir / 'features_engineered_v3.pkl'
        ...
        # Save summary (V3: distinct filename, don't overwrite V2's summary)
        summary_file = self.output_dir / 'processing_summary_v3.txt'
        with open(summary_file, 'w') as f:
            f.write(f"LLRF Data Preparation Summary (V3 -- ground-truth labels)\n")
            f.write(f"=" * 80 + "\n")
