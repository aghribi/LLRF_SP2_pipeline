#!/usr/bin/env bash
#SBATCH --job-name=saliency_cnn_lstm_v7
#SBATCH --output=logs/saliency_cnn_lstm_v7.%j.out
#SBATCH --error=logs/saliency_cnn_lstm_v7.%j.err
...
echo "Running saliency_cnn_lstm against V7 | Job ${SLURM_JOB_ID} | $(date)"
cd ${PROJECT_DIR}
${PYTHON_ENV} ${PROJECT_DIR}/pipeline/00_scripts/extract_saliency_phaseD.py --arch cnn_lstm
EXIT_CODE=$?
