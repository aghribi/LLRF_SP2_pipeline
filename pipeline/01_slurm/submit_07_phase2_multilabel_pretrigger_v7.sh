#!/bin/bash
#SBATCH --job-name=step_07_pretrigger_v7
#SBATCH --output=logs/step_07_pretrigger_v7_%j.out
#SBATCH --error=logs/step_07_pretrigger_v7_%j.err
#SBATCH --time=2:00:00
...
echo "STEP 07: Phase 2 - Multi-Label (pre-trigger only) | Job ${SLURM_JOB_ID} | $(date)"
...
${PYTHON_ENV} pipeline/00_scripts/prepare_07_phase2_multilabel.py \
    --input ${COOKED_DATA} \
    --output ${COOKED_DATA}/step_07_phase2_pretrigger_v7 \
    --pretrigger
EXIT_CODE=$?
