#!/usr/bin/env bash
#SBATCH --job-name=saliency_cnn_v6
#SBATCH --output=logs/saliency_cnn_v6.%j.out
#SBATCH --error=logs/saliency_cnn_v6.%j.err
...
echo "Running saliency_cnn against V6 | Job ${SLURM_JOB_ID} | $(date)"
cd ${PROJECT_DIR}
${PYTHON_ENV} ${PROJECT_DIR}/pipeline/00_scripts/extract_saliency_phaseD.py --arch cnn
EXIT_CODE=$?
