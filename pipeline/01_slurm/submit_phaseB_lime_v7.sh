#!/usr/bin/env bash
#SBATCH --job-name=phaseB_lime_v7
#SBATCH --output=logs/phaseB_lime_v7.%j.out
#SBATCH --error=logs/phaseB_lime_v7.%j.err
...
echo "Running phaseB_lime against V7 | Job ${SLURM_JOB_ID} | $(date)"
cd ${PROJECT_DIR}
${PYTHON_ENV} ${PROJECT_DIR}/pipeline/00_scripts/extract_lime_phaseB_cc.py
EXIT_CODE=$?
