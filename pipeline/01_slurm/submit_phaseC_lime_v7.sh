#!/usr/bin/env bash
#SBATCH --job-name=phaseC_lime_v7
#SBATCH --output=logs/phaseC_lime_v7.%j.out
#SBATCH --error=logs/phaseC_lime_v7.%j.err
...
echo "Running phaseC_lime against V7 | Job ${SLURM_JOB_ID} | $(date)"
cd ${PROJECT_DIR}
${PYTHON_ENV} ${PROJECT_DIR}/pipeline/00_scripts/extract_lime_phaseC_lp.py
EXIT_CODE=$?
