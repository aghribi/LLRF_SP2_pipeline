#!/usr/bin/env bash
# SLURM submit script for Phase D (Deep multi-head training, CNN arch), against V6 dataset
# Usage: sbatch submit_10_phaseD_transformer_v6.sh

#SBATCH --job-name=phaseD_transformer_v6
#SBATCH --output=logs/phaseD_transformer_v6.%j.out
#SBATCH --error=logs/phaseD_transformer_v6.%j.err
...
cd "$PROJECT_ROOT"
"$PY_BIN" "$SCRIPT" --arch transformer --epochs 50 --batch-size 128 --device cpu
