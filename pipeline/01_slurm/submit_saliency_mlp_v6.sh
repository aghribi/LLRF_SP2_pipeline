#!/usr/bin/env bash
#SBATCH --job-name=saliency_mlp_v6
#SBATCH --output=logs/saliency_mlp_v6.%j.out
#SBATCH --error=logs/saliency_mlp_v6.%j.err
...
echo "Running saliency_mlp against V6 | Job ${SLURM_JOB_ID} | $(date)"
cd ${PROJECT_DIR}
${PYTHON_ENV} ${PROJECT_DIR}/pipeline/00_scripts/extract_saliency_phaseD.py --arch mlp
EXIT_CODE=$?
