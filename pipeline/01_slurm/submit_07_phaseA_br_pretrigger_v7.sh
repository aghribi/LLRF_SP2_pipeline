#!/bin/bash
#SBATCH --job-name=step_07A_pretrigger_v7
#SBATCH --output=logs/step_07_phaseA_pretrigger_v7_%j.out
#SBATCH --error=logs/step_07_phaseA_pretrigger_v7_%j.err
...
echo "STEP 07: Phase A - BR baseline (pre-trigger only) | Job ${SLURM_JOB_ID} | $(date)"
...
python pipeline/00_scripts/phaseA_br_test.py \
    ...
    --output ${COOKED_DATA}/step_07_phaseA_pretrigger_v7_br \
    --pretrigger
EXIT_CODE=$?
