...
Binary classification (fault vs. normal) is performed on the full event window (post-trigger included) to establish a baseline for the supervised steps that follow, using the complete \MetricZeroSixBinaryClassificationNFeatures{}-feature set. Logistic Regression, Random Forest, XGBoost, and SVM (RBF kernel) are compared; this task's classes are close to balanced (Sec.~\ref{sec:introduction}), so no class weighting is applied at this stage (Appendix~\ref{appendix:hyperparameters}). Near-ceiling performance is expected for this specific task, not itself evidence of a leak: the full post-trigger window includes the interlock-trip transient by construction, which is close to trivially separable from normal operation on amplitude alone -- this is a materially easier task than the precursor-detection task in Sec.~\ref{sec:system_description}, which restricts to the pre-trigger window specifically to avoid this. Results are reported in Sec.~\ref{sec:results}.
...
RF regulation out of tolerance & \MetricZeroNinecEnhancedSubclassDiscoveryRfRegulationOutOfToleranceBestK & \MetricZeroNinecEnhancedSubclassDiscoveryRfRegulationOutOfToleranceSilhouette \\
\bottomrule
\end{tabular*}
\end{table}
