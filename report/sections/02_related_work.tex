...
Root cause analysis (RCA) in multi-fault scenarios is a broader research area spanning IT systems, manufacturing, and process industries, commonly built on causal inference methods~\cite{pearl_causality} or Bayesian networks. These approaches typically require extensive domain knowledge encoding or large datasets with labeled root causes -- a requirement this pipeline's own root-cause task does not yet meet (Sec.~\ref{sec:methodology}), which is why we do not claim to have implemented causal inference here.
...
In accelerator physics, explainability is crucial for operator trust, regulatory compliance, and system validation. Related work at the European X-ray Free Electron Laser (DESY) has applied probabilistic anomaly-detection models to accelerator fault diagnosis~\cite{nawaz2021probabilistic}.
