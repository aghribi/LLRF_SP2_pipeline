#!/usr/bin/env python3
"""Run Phase A SHAP analysis headless: create per-label summary and top-20 plots and save metrics CSV."""
import os
...
logger = logging.getLogger('phaseA_shap_run')
...
for i, lbl in enumerate(label_names):
    ...
    shap_vals = sh[key]
    try:
        # shap.summary_plot (use original array)
        ...
        if isinstance(shap_vals, list):
            # list of class-wise arrays: prefer last class, else mean across classes
            if len(shap_vals) >= 2:
            ...
            if arr.ndim == 2:
                ...
                # collapse trailing axes (e.g., class dimension) by selecting last class when binary,
                # otherwise average across the extra dimensions
                if arr.shape[-1] == 2:
                    shap_2d = arr[..., 1]
                else:
                    shap_2d = np.mean(arr, axis=tuple(range(2, arr.ndim)))
            else:
                # unexpected shape, attempt to reshape to (n_samples, n_features)
                shap_2d = arr.reshape((arr.shape[0], -1))

        # Now compute mean absolute per feature
        ...
        ax.set_title(f'Top 20 features by mean |SHAP| (label {i} - {lbl})')
...
rows = []
if br:
    rows.append({'model':'BR','hamming':br.get('hamming_loss'),'micro_f1':br.get('micro_f1'),'macro_f1':br.get('macro_f1')})
if cc:
    rows.append({'model':'CC','hamming':cc.get('hamming_loss'),'micro_f1':cc.get('micro_f1'),'macro_f1':cc.get('macro_f1')})
if rows:
    df = pd.DataFrame(rows).set_index('model')
    csv_path = OUT_DIR / 'phaseA_metrics_comparison.csv'
    df.to_csv(csv_path)
    logger.info('Saved metrics comparison to %s', csv_path)
    print(df)
else:
    logger.warning('No BR/CC metrics found in summary')
