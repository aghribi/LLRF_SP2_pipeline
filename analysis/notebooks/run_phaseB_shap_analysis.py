#!/usr/bin/env python3
"""Run Phase B SHAP analysis headless: create per-label summary and top-20 plots and save metrics CSV."""
import os
...
logger = logging.getLogger('phaseB_shap_run')

COOKED = Path('/sps/m4cast/_spiral2_data/_llrf_data/cooked_data')
PHASEB = COOKED / 'step_08_phaseB_cc'
SHAP_FILE = PHASEB / 'shap_cc.npz'
SUMMARY_FILE = PHASEB / 'phaseB_summary.pkl'
OUT_DIR = Path('analysis/outputs/phaseB_shap')
...
if rows:
    ...
    csv_path = OUT_DIR / 'phaseB_metrics_comparison.csv'
    df.to_csv(csv_path)
